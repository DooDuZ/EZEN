/**
 	함수 : 미리 정의된 코드
 		기본 형태 : 1. 매개변수 2. 정의 코드 3. 결과[리턴값]
 		사용 목적 : 1. 재활용 2. 매모리 효율성 3. 매개변수에 따른 서로 다른 리턴값
 		function 함수명 ( 매개변수 ~ ){	정의코드	return	}
 	...
 	5. 콜백함수 : 함수도 자료형이기 때문에 매개변수로 전달이 가능하다.
		...
		* 미리 만들어진 콜백을 사용하는 배열 내 함수들
			forEach()	: 반복문
			map()		: 새로운 배열 생성[ return 값을 새로운 배열 저장]
			filter		: 새로운 배열 생성[ return true인 경우에만 값을 새로운 배열 저장 ]
