...
function 버튼클릭(i){									//1. 함수 선언 // 현재 단계에선 아직 매개변수 필요 없음	
	...
	while(true){
		rand = Math.floor(Math.random()*9)			//4. 0이상 1미만의 난수를 만들어주는 Math.random()에 9를 곱하여 0~8.9999... 의 난수를 만든 후 Math.floor를 이용해 소수점 버림.
		if(!중복체크(rand)){
			break
		}
	}
...
function 승리판단(){
	for(let i = 0 ; i<=6 ; i += 3){
		if(진행상황[i]!=null && 진행상황[i] == 진행상황[i+1] && 진행상황[i+1] == 진행상황[i+2]){
			return true
		}
	}for(let i = 0 ; i <= 2 ; i++){
		if(진행상황[i]!=null && 진행상황[i] == 진행상황[i+3] && 진행상황[i+3] == 진행상황[i+6]){
			return true
		}
	}
	let i = 2
	if( 진행상황[i]!= null && 진행상황[i] == 진행상황[i+2] && 진행상황[i+2] == 진행상황[i+4]){
		return true
	}
	i=0
	if(진행상황[i]!= null && 진행상황[i] == 진행상황[i+4] && 진행상황[i+4] == 진행상황[i+8]){
		return true
	}	
}
